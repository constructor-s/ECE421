...
def buildGraph2(beta1, beta2, epsilon, 
               lossType=None, learning_rate=0.001, d=784, stddev=0.5):
...
#            W = np.random.RandomState(42).rand(trainDataVec.shape[0], 1)
 ...
 # %% Q3.4
    ...
    def tf_train2(beta1, beta2, epsilon, 
           lossType=None, learning_rate=0.001, stddev=0.5, 
           batch_size=500, lambd_val=0, tf_epochs=tf_epochs):
        
        print('\nbeta1', 'beta2', 'epsilon', 'lossType', 'learning_rate', 'stddev', 'batch_size', 'lambd_val', 'tf_epochs', sep='\t')
        print(beta1, beta2, epsilon, lossType, learning_rate, stddev, batch_size, lambd_val, tf_epochs, sep='\t')
        
        optimizer, W, b, x, y, lambd, yhat, loss = buildGraph2(beta1, beta2, epsilon, 
           lossType=lossType, learning_rate=learning_rate, stddev=stddev)
        ...
        with tf.Session() as sess:
            ...
            print('iter', 'training accuracy', 'validation accuracy', 'testing accuracy', sep='\t')
            for i in range(tf_epochs):
                valid_loss, valid_yhat = sess.run([
                        loss, yhat
                        ], feed_dict=valid_dict)
                ...
                if i % 10 == 0 or i == tf_epochs-1:
                    print('\r', i, '%.3f' % train_acc, '%.3f' % valid_acc, '%.3f' % test_acc, sep='\t', end='    \r')
    ...
    print('\npart a')
    for b in beta1:
        tf_train2(beta1=b, beta2=beta2_default, epsilon=epsilon_default, 
               lossType=None, learning_rate=0.001, stddev=0.5, 
               batch_size=500, lambd_val=0, tf_epochs=tf_epochs)
    print('\npart b')
    for c in beta2:
        tf_train2(beta1=beta1_default, beta2=c, epsilon=epsilon_default, 
               lossType=None, learning_rate=0.001, stddev=0.5, 
               batch_size=500, lambd_val=0, tf_epochs=tf_epochs)
    print('\npart c')
    for e in epsilon:
        tf_train2(beta1=beta1_default, beta2=beta2_default, epsilon=e, 
               lossType=None, learning_rate=0.001, stddev=0.5, 
               batch_size=500, lambd_val=0, tf_epochs=tf_epochs)